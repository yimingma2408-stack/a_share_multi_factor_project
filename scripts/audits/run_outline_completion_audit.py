...
def collect_requirements() -> list[Requirement]:
    ...
    return [
        ...
        Requirement("reproducibility", "Installable, dependency-pinned environment and explicit formal-EOT cache policy", ("pyproject.toml", "requirements-lock.txt", "environment.yml", "scripts/workflows/run_full_research_pipeline.py"), "complete" if package_complete else "incomplete", "`--full --formal-eot reuse|rerun|skip` explicitly controls final formal EOT handling."),
        Requirement("engineering", "Current-state tests and formal artifact audit", ("tests/test_eot_map_lifecycle_test.py", "tests/test_data_quality_and_attribution.py", "scripts/audits/audit_formal_eot_delivery.py"), "complete" if exists("scripts/audits/audit_formal_eot_delivery.py") else "incomplete", "Run `pytest` and `python scripts/audits/audit_formal_eot_delivery.py` in the installed environment."),
    ]
